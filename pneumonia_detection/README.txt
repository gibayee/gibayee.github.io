...
- **A Streamlit Web Interface**: This allows users to upload X-ray images.
...
1. **Streamlit Web Interface**
   - The front end of the application provides users with an easy-to-use interface where they can upload X-ray images. The interface is built with Streamlit.
...
- **Streamlit**: A Python-based web framework that is used for creating the web interface for data science projects
...
- **index.html**: This file contains the backend logic of the application, including routes for uploading images and interacting with the model.
- **data source.txt**: contains the link to the data used for this project
- **pneumonia detection.ipynb**: a jupyter notebook containing all the codes for the pneumonia detection
- **/models/imageclassifier.h5**: Defines and compiles the CNN model for pneumonia detection.
...
- **Why Streamlit for the Web Interface?**: Streamlit was chosen due to its simplicity and flexibility in building small-scale web applications. It integrates well with the Python-based machine learning pipeline and serves as a robust backend for the project.
